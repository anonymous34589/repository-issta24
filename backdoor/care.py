...
class causal_analyzer:
    ...
    def analyze(self, dd_gen, cex_gen):
        ...
        for step in range(self.steps):
            ...
            result, acc = self.pso_test_repair_set(self.r_weight, self.target)
            print("after repair on repair set: attack SR: {}, BE acc: {}".format(result, acc))
            result, acc = self.pso_test(self.r_weight, self.target)
            print("after repair on test set: attack SR: {}, BE acc: {}".format(result, acc))
    ...
    def pso_test_repair_set(self, r_weight, target):
        result = 0.0
        correct = 0.0
        tot_count = 0
        if len(self.rep_index) != 0:
